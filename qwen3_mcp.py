...
# from dotenv import load_dotenv  # 不再需要.env文件
...
class Configuration:
    ...
    def __init__(self) -> None:
        # 直接在代码中配置 API key, base_url 和 model
        self.api_key = "sk-a6aaf8beba18425e9942c4a33ae58caf"
...
class Server:
    ...
    async def initialize(self) -> None:
        ...
        if command is None:
            raise ValueError("command field is required in server config")
    
        server_params = StdioServerParameters(
            command=command,
            args=self.config["args"],
            env={**os.environ, **self.config["env"]} if self.config.get("env") else None,
            # 添加编码参数，使用 GBK 或 CP936 编码
            encoding="utf-8"  # 修复编码错误
        )
        try:
...
class MultiServerMCPClient:
    ...
    async def create_function_response_messages(self, messages: List[Dict[str, Any]], response: Any) -> List[
        Dict[str, Any]]:
        """
